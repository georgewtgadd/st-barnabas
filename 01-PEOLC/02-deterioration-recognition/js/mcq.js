...
function showMCQResults() {
  ...
  window._mcqRaw = correctCount;
  window._mcqTotal = _mcqAnswers.length;

  // Report to the LRS, if one is present (see js/xapi.js)
  if (typeof XAPI !== 'undefined') {
    XAPI.setScore('mcq', "What Would You Do? — Case Study Assessment", correctCount, 0, _mcqAnswers.length, passed);
  }
}
...
function retryMCQ() {
  ...
  if (typeof XAPI !== 'undefined') { XAPI.loggedReset('mcq', 'MCQ retry'); }
  renderMCQ();
}
