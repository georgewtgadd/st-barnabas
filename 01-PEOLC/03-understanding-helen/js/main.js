...
document.addEventListener('pagesLoaded', () => {
  ...
  document.addEventListener('pageChanged', e => {
    ...
    if (e.detail.page === 7 && typeof populateAcpSophieNote === 'function') populateAcpSophieNote();
  });

  // ── Modal close — always routes through each modal's own close
  //    function (not just classList removal), so unlock checks like
  //    "all hotspots found" / "all pillars explored" always run,
  //    however the learner closes the modal.
  const modalClosers = {
    'pillar-modal':      () => closePillarModal(),
    'video-modal':       () => closeVideoModal(),
    'hs-modal':          () => closeHotspotModal(),
    'acp-notes-modal':   () => closeAcpModal(),
  };

  function closeModalById(id) {
    const closer = modalClosers[id];
    if (closer) closer();
  ...
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape') {
      document.querySelectorAll('.modal-backdrop.open').forEach(m => closeModalById(m.id));
    }
  });
