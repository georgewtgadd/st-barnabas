...
function answerQuiz(i) {
  ...
  window._quizAnswers = window._quizAnswers || [];
  window._quizAnswers[quizIndex] = { prompt: q.prompt, chosen: q.options[i], correct };

  // Report this individual answer to the LRS, if one is present (see js/xapi.js)
  if (typeof XAPI !== 'undefined') {
    XAPI.responded('quiz/q' + (quizIndex + 1), q.prompt, q.options[i], {
      'https://www.stbarnabashospice.co.uk/xapi/extensions/correct': correct
    });
  }
...
function renderQuizResults() {
  ...
  // Track for the Learning Record (see js/record.js)
  window._quizScoreText = quizScore + ' / ' + total + ' (' + pct + '%)';
  window._quizPassed = strong;
  ...
  card.innerHTML = `
    <div class="quiz-results">
      ...
      <div style="display:flex; gap:14px; justify-content:center; flex-wrap:wrap;">
        ...
        <button class="btn btn-primary" onclick="goToPage(9)" aria-label="Continue to My Learning Record">
          My Learning Record <span aria-hidden="true">→</span>
        </button>
      </div>
    </div>
  `;

  // Report the score and mark the quiz activity complete with the LRS, if one is present
  if (typeof XAPI !== 'undefined') {
    XAPI.setScore('quiz', 'Check Your Understanding', quizScore, 0, total, strong);
  }
}
