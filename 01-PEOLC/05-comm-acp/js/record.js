...
function populateLearningRecord() {
  ...
  const matchScore = document.getElementById('rec-match-score');
  const matchR     = window._matchResult;
  if (matchScore) matchScore.textContent = matchR === 'correct' ? 'All documents matched correctly ✓' : matchR === 'incorrect' ? 'Attempted — some incorrect' : 'Not attempted';
  if (matchBadge) {
    matchBadge.textContent = matchR === 'correct' ? 'Completed' : matchR === 'incorrect' ? 'Partial' : '—';
    matchBadge.className   = 'record-badge ' + (matchR === 'correct' ? 'badge-pass' : matchR === 'incorrect' ? 'badge-done' : '');
  }
...
function exportLearningRecord() {
  ...
  const lines = [
    ...
    'ACP TOOL MATCHING',
    '────────────────────────────────────────────────',
    'Result: ' + (window._matchResult === 'correct' ? 'All documents matched correctly' : window._matchResult || 'Not attempted'),
    '',
