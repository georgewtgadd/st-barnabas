...
function populateLearningRecord() {
  ...
  if (capScore) capScore.textContent = capR === 'correct'
    ? 'Correct sequence identified: Understand -> Retain -> Weigh Up -> Communicate (correct)'
  ...
  // ── Scenario (branching — path & ending vary by learner choices) ──
  const scenEndingTitle = window._scenarioEndingTitle;   // e.g. "Best Interests, Truly Met"
  const scenSuccess     = window._scenarioSuccess;        // true | false | undefined (not yet reached)
  const scenPath        = window._scenarioPath || [];     // [{ label, optionLabel, correct }]
  const scenBadge = document.getElementById('rec-scen-badge');
  const scenScore = document.getElementById('rec-scen-score');
  const scenPathEl   = document.getElementById('rec-scen-path');
  const scenEndingEl = document.getElementById('rec-scen-ending');

  if (scenPathEl) {
    scenPathEl.textContent = scenPath.length
      ? scenPath.map(s => s.label + ' (' + s.optionLabel + (s.correct ? ', on track' : ', off track') + ')').join(' -> ')
      : '—';
  }
  if (scenEndingEl) scenEndingEl.textContent = scenEndingTitle || '—';

  if (scenScore) scenScore.textContent = scenEndingTitle
    ? (scenSuccess ? 'Reached a successful outcome ✓' : 'Reached a weaker outcome — worth resetting and trying a different path')
    : 'Not completed';
  if (scenBadge) {
    scenBadge.textContent = scenEndingTitle ? (scenSuccess ? 'Completed' : 'Completed (weaker outcome)') : '—';
    scenBadge.className   = 'record-badge ' + (scenEndingTitle ? (scenSuccess ? 'badge-pass' : 'badge-done') : '');
  }
...
function exportLearningRecord() {
  ...
  doc.text('Palliative & End of Life Care · Module 5', MARGIN, 30);
  ...
  row('Series', 'Palliative & End of Life Care · Module 5');
  ...
  row('Path taken', (document.getElementById('rec-scen-path') || {}).textContent || '—');
  row('Ending reached', (document.getElementById('rec-scen-ending') || {}).textContent || '—');
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    doc.text('St Barnabas Hospice', MARGIN, 291);
...
function finishModule() {
  if (typeof XAPI !== 'undefined') {
    XAPI.setCompletion('', 'Communication & Advance Care Planning', {
