...
function openPhaseDetail(type, num) {
  ...
  // Mark button active — layers now have two representations (the ring
  // in the SVG diagram and the legend row below it), so mark both via
  // data-layer; phases only have one (the timeline node), so id is enough.
  if (type === 'phase') {
    const btn = document.getElementById('phase-btn-' + num);
    if (btn) btn.classList.add('active');
  } else {
    document.querySelectorAll('[data-layer="' + num + '"]').forEach(el => el.classList.add('active'));
  }
  ...
  if (hint && type === 'phase') {
    ...
    hint.textContent = remaining > 0
      ? 'Explore ' + remaining + ' more phase(s) to get the most out of this page.'
...
function unlockPhasesContinue() {
  // Practice activity — completing the recommended minimum is encouraged
  // but no longer required to proceed. _phasesDone stays accurate for the
  // Learning Record; the Continue button is available from the start.
  _phasesDone = phasesOpened.size >= PHASES_REQUIRED;
}
