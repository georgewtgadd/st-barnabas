...
function _lockSortContinue() {
  // Practice activity — completing it is encouraged but no longer required
  // to proceed. _sortDone stays accurate for the Learning Record; the
  // Continue button stays available either way.
  _sortDone = false;
  const lock = document.getElementById('sort-locked-msg');
  const btn  = document.getElementById('sort-continue-btn');
  if (lock) lock.style.display = 'none';
