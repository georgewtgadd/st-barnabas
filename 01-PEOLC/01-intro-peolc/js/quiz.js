...
function answerQuiz(choice) {
  ...
  if (trueBtn) {
    if (item.answer === true)       trueBtn.classList.add('correct-glow');
    else if (choice === true)       trueBtn.classList.add('wrong-glow');
  }
  if (falseBtn) {
    if (item.answer === false)      falseBtn.classList.add('correct-glow');
    else if (choice === false)      falseBtn.classList.add('wrong-glow');
  }
...
function showResults() {
  ...
  if (typeof XAPI !== 'undefined') {
    XAPI.setScore('quiz', 'Knowledge Check (True or False)', pct, 0, 100, passed);
  }
}
