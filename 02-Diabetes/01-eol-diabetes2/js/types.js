/* types.js — Data and Controller for the Diabetes Spectrum cards */
'use strict';
...
/* ── State ─────────────────────────────────────────── */
let _typesVisited = new Set();
const TOTAL_TYPES = 6;

/* ══ MODAL CONTROLS ══════════════════════════════════ */
function openTypeModal(id) {
  const data = TYPES_DATA[id];
  if (!data) return;

  const modal  = document.getElementById('type-modal');
  const stripe = document.getElementById('tm-stripe');
  const badge  = document.getElementById('tm-badge');
  const tag    = document.getElementById('tm-tag');
  const title  = document.getElementById('tm-title');
  const body   = document.getElementById('tm-body');

  if (!modal || !body) return;

  // Set visual theme based on ID
  stripe.className = 'type-modal-stripe tms-' + id;
  badge.className  = 'type-modal-badge tmb-' + id;
  badge.textContent = data.modalTag;
  ...
  data.sections.forEach(s => {
    html += `<div class="tm-section">
      <div class="tm-section-label">${s.label}</div>
      <div class="tm-section-body">${s.body}</div>
    </div>`;
  });
  html += `<div class="tm-eol-box">
    <div class="tm-eol-label">EoL Relevance</div>
    <div class="tm-eol-body">${data.eol}</div>
  </div>`;
...
/* ── Progress Tracking ─────────────────────────────── */
function _markTypeVisited(id) {
  _typesVisited.add(id);

  // Update UI card
  const card = document.getElementById('tc-' + id);
  const mark = document.getElementById('tv-' + id);
  if (card) card.classList.add('visited');
  if (mark) mark.hidden = false;

  // Check if all explored
  if (_typesVisited.size >= TOTAL_TYPES) {
    ...
    if (hint) hint.textContent = 'All classifications explored. You can now continue.';
  }
}
